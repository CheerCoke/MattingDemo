...
public class StickerView extends FrameLayout {
    ...
    private DashPathEffect dashEffect = new DashPathEffect(new float[]{10f, 10f}, 0f);
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        shadowPaint.setStrokeWidth(2f); // 阴影宽度
        shadowPaint.setColor(Color.argb(100, 0, 0, 0)); // 半透明黑色阴影
...
//                dashedLinePaint.setPathEffect(dashEffect);
//                dashedLinePaint.setStrokeWidth(2f);
//                dashedLinePaint.setColor(Color.parseColor("#4DFFFFFF"));
//            } else {
//                dashedLinePaint.setStrokeWidth(3f);
//                dashedLinePaint.setPathEffect(null);
//                dashedLinePaint.setColor(Color.parseColor("#CCFA6400"));
//            }
//
////            canvas.drawLine(0f, centerY, getWidth(), centerY, shadowPaint);
//
//            canvas.drawLine(0f, centerY, getWidth(), centerY, dashedLinePaint);
//
//            if (!isTouchingVertical) {
//                dashedLinePaint.setStrokeWidth(2f);
//                dashedLinePaint.setPathEffect(dashEffect);
//                dashedLinePaint.setColor(Color.parseColor("#4DFFFFFF"));
//            } else {
...
//            }
//
////            canvas.drawLine(centerX, 0, centerX, getHeight(), shadowPaint);
//
//            canvas.drawLine(centerX, 0, centerX, getHeight(), dashedLinePaint);
//        }
        ...
        switch (currentMode) {
            ...
            case ActionMode.DRAG:
                if (handlingSticker != null) {
                    ...
                    if (handlingSticker.isTranslateEnable()) {
                    ...
                    PointF centerPoint = handlingSticker.getMappedCenterPoint();

                    Log.e("StickerView", "CenterPoint:" + centerPoint + " (" + getWidth() + "," + getHeight() + ")");
            ...
            case ActionMode.ZOOM_WITH_TWO_FINGER:
                if (handlingSticker != null) {
                    ...
                    moveMatrix.set(downMatrix);
                    if (handlingSticker.isScaleEnable()) {
                    ...
                    if (handlingSticker.isRotateEnable()) {
//                        moveMatrix.postRotate(newRotation - oldRotation, midPoint.x, midPoint.y);
                        handlingSticker.setCurrentAngle(newRotation);
                        onStickerOperationListener.onStickerRotated(handlingSticker);
                    }
                    handlingSticker.setMatrix(moveMatrix);
                }
    ...
    public StickerView addSticker(@NonNull final Sticker sticker,
        ...
        if (isLaidOut()) {
            addStickerImmediately(sticker, new Pair<>(offsetX, offsetY));
        } else {
            post(new Runnable() {
                @Override
                public void run() {
    ...
    public StickerView addSticker(@NonNull final Sticker sticker, float scale,
                                  final Float offsetX, final Float offsetY, float degree) {
        if (isLaidOut()) {
            addStickerImmediately(sticker, scale, degree, new Pair<>(offsetX, offsetY));
        } else {
            post(new Runnable() {
                @Override
                public void run() {
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, final Pair<Float, Float> offset) {
        setStickerPosition(sticker, offset);
...
//                .postScale(scaleFactor / 2, scaleFactor / 2, getWidth() / 2, getHeight() / 2);
    ...
    protected void addStickerImmediately(@NonNull Sticker sticker, Float scale, Float degree, final Pair<Float, Float> offset) {

        sticker.getMatrix().postScale(scale, scale, sticker.getWidth() / 2f, sticker.getHeight() / 2f);
        sticker.getMatrix().postRotate(degree, sticker.getWidth() / 2f, sticker.getHeight() / 2f);
        setStickerPosition(sticker, offset);
...
//                .postScale(scaleFactor / 2, scaleFactor / 2, getWidth() / 2f, getHeight() / 2f);
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, Pair<Float, Float> offset) {
        float width = getWidth();
        float height = getHeight();
        float offsetX = width * offset.first - sticker.getWidth() / 2f;
        float offsetY = height * offset.second - sticker.getHeight() / 2f;
     ...
     * 打乱stickers的位置
     */
    public void shutterStickers() {
        int totalCount = stickers.size();
        for (int index = 0; index < totalCount; index++) {
            float offsetX = (index % 2 == 1) ? 0.3f : 0.7f;
            offsetX += (float) (new Random().nextDouble() * 0.3 - 0.3);

            float offsetY = 0.5f * index / totalCount + (float) (new Random().nextDouble() * 0.4 - 0.2);
            offsetY = (index > totalCount / 2) ? 0.5f + offsetY : 0.5f - offsetY;

            Sticker bitmapSticker = stickers.get(index);
            List<Float> rotateDegreesList = Arrays.asList(0f, 4f, 6f, -4f, -6f, 8f, -8f);
            float rotateDegrees = rotateDegreesList.get(new Random().nextInt(rotateDegreesList.size()));

            List<Float> scaleList = Arrays.asList(0.5f, 0.75f, 0.95f, 1f, 1.2f, 1.5f);
            float scale = scaleList.get(new Random().nextInt(scaleList.size()));
